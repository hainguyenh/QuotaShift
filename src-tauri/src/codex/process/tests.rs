...
fn test_is_codex_cli_process() {
    ...
    assert!(is_codex_cli_process(
        "cmd.exe",
        "\"C:\\Program Files\\Codex\\codex.exe\" --help"
    ));
    assert!(!is_codex_cli_process("decodex.exe", "decodex.exe"));
    assert!(!is_codex_cli_process(
        "chrome.exe",
        "chrome.exe https://codex.com"
    ));
}
...
fn test_is_codex_ide_extension_process() {
    assert!(is_codex_ide_extension_process(
        "codex-language-server.exe",
        "codex-language-server.exe --stdio"
    ));
    assert!(is_codex_ide_extension_process(
        "codex-agent",
        "/path/to/codex-agent"
    ...
    assert!(is_codex_ide_extension_process(
        "node.exe",
        "node C:\\Users\\user\\.vscode\\extensions\\openai.chatgpt-0.1.0\\dist\\server.js"
    ));
    assert!(is_codex_ide_extension_process(
        "node",
        "/home/user/.vscode/extensions/codex-team.codex/out/extension.js"
    ));
    assert!(!is_codex_ide_extension_process(
        "code.exe",
        "code.exe C:\\my-project"
    ));
    assert!(!is_codex_ide_extension_process(
        "language_server.exe",
        "language_server.exe"
...
fn test_is_target_codex_process_safeguards() {
    ...
    assert!(!is_target_codex_process(
        12345,
        current_pid,
        "codex.exe",
        "codex.exe"
