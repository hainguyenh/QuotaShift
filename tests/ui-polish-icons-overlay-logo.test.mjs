...
const uiAdjustmentCss = read("src/styles/ui-adjustment.css");
...
test("overlay surface is clipped to its own viewport", () => {
  assert.match(uiAdjustmentCss, /\.overlay-container\s*\{[\s\S]*overflow:\s*hidden/);
  assert.match(uiAdjustmentCss, /\.glass-card\s*\{[\s\S]*max-width:\s*100%/);
  assert.match(uiPolish, /\.glass-card\s*\{[\s\S]*max-height:\s*100%/);
