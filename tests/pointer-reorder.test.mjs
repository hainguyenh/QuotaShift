...
test('reorders symmetrically with realistic card dimensions and handle grab offset', () => {
  ...
  assert.deepEqual(
    reorderIdsAtPointer(['acc1', 'acc2'], 'acc1', realisticRects, 205, 15),
    ['acc2', 'acc1'],
  );
  // Staying above boundary (y = 190): visual center = 190 - 15 + 80 = 255 < 265 -> remains in slot 0
  assert.deepEqual(
    reorderIdsAtPointer(['acc1', 'acc2'], 'acc1', realisticRects, 190, 15),
    ['acc1', 'acc2'],
  );
  ...
  assert.deepEqual(
    reorderIdsAtPointer(['acc1', 'acc2'], 'acc2', realisticRects, 195, 15),
    ['acc2', 'acc1'],
  );
  // Staying below boundary (y = 210): visual center = 210 - 15 + 80 = 275 >= 265 -> remains in slot 1
  assert.deepEqual(
    reorderIdsAtPointer(['acc1', 'acc2'], 'acc2', realisticRects, 210, 15),
    ['acc1', 'acc2'],
  );
});
