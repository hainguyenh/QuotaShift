...
const tray = await readFile(new URL("../src-tauri/src/window/window_manager.rs", import.meta.url), "utf8");
...
test("adding a Codex account does not mark it active before Apply", () => {
  ...
  assert.doesNotMatch(modalSlice, /onAccountAdded=\{async \(id\) => \{\s*setActiveCodexId\(id\);\s*localStorage\.setItem\(CODEX_ACTIVE_ID_KEY, id\)/);
});
...
test("tracked Codex tooltip survives Antigravity polling failure", () => {
  const errorBranch = tray.slice(tray.indexOf("Err(_) =>"), tray.indexOf("pub fn update_tray_only"));
  assert.match(errorBranch, /monitored_codex\.is_some\(\)/);
  assert.match(errorBranch, /format_tooltip\(&status\)/);
