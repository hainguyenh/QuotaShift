...
test("overlay CSS uses one fixed surface and uniform visual scale", () => {
  const adjustment = read("src/styles/ui-adjustment.css");
  const css = adjustment + read("src/styles/overlay.css");
  ...
  assert.match(cardRule, /height:\s*100%/);
  assert.match(cardRule, /overflow:\s*hidden/);
  assert.doesNotMatch(adjustment, /--overlay-card-width|--overlay-card-height|--overlay-ui-scale-inverse/);
...
test("Codex usage refresh restores reset credits without making normal usage fragile", () => {
  ...
  assert.match(overlay, /typeof data\.resetCount === ["']number["']\s*&&\s*data\.resetCount\s*>\s*0/);
});
