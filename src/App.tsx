...
import { invoke } from "@tauri-apps/api/core"; import { listen, emit } from "@tauri-apps/api/event"; import { openUrl } from "@tauri-apps/plugin-opener"; import { getVersion } from "@tauri-apps/api/app"; import { useGlobalShortcuts } from "./utils/common/use-global-shortcuts";
import type { OverlayAccountData, OverlayQuotaRow } from "./components/overlay/OverlayApp"; import type { AntigravityAccount, AntigravityUsageCacheEntry, AntigravityWorkerProgress, CodexAccount, CodexAccountPool, CodexModelCatalogCacheEntry, CodexRouterStatus, FullStatus } from "./utils/common/types";
import { deobfuscate, obfuscate, decodeJwtProfile } from "./utils/auth/auth"; import { isUsageCacheFresh, pickBestCodexPoolMember, findCodexPoolFailover, reconcileCodexPools, isCodexModelCacheFresh, normalizeCodexModelCatalog, buildCodexRouterConfig, normalizeCodexUsageWindows, markAccountLastUsed, savePersistentWorkerPreference, pickBestAntigravityAccount, pickBestCodexAccount, classifyCodexTier, isCodexAccountOAuth } from "./utils";
...
import { refreshAntigravityAccountsCloudFirst as refreshAntigravityCloudOps } from "./utils/antigravity/app-antigravity-ops"; import { fetchCodexUsageData } from "./utils/codex/app-codex-ops"; import { buildClaudeOverlayPayload, buildAntigravityOverlayPayload } from "./utils/common/app-overlay-helpers"; import { saveAccountOrder, sortByOrder } from "./utils/account/account-order"; import { buildBackupData, encryptBackup, decryptBackup, restoreBackupData } from "./utils/common/app-backup"; import { isNewerVersion } from "./utils/common/update-policy";
...
export const App: React.FC = () => {
  ...
  const { localAntigravitySession, updateLocalSessionFromStatus, handleLocalAntigravitySessionCaptured, handleAddLocalSessionToMonitored, handleApplyAccountToLocalSession, syncLocalSessionFromDisk } = useLocalSession(antigravityAccounts, setAntigravityAccounts, refreshAntigravityAccountsCloudFirst);
  ...
  const triggerRefresh = async (force = false) => {
    ...
    try {
      const s = await invoke<any>("force_refresh"); if (s) { setLastFullStatus(s); updateLocalSessionFromStatus(s); } await syncLocalSessionFromDisk();
  ...
  useGlobalShortcuts(handleToggleOverlay, () => triggerRefresh(true));
  ...
  const handlePassphraseSubmit = async (passphrase: string) => {
    if (passMode === "export") {
      try {
        const data = { ...buildBackupData(antigravityAccounts, codexAccounts, isDarkMode ? "dark" : "light"), codex: { accounts: codexAccounts, pools: loadCodexPools() } }, enc = await encryptBackup(data, passphrase), path = await invoke<string>("export_backup_file", { content: enc });
  ...
  const handleCheckUpdate = async (latestTag = "v1.0.1") => { setUpdateAvailable(true); setUpdateTag(latestTag); setUpdatePromptOpen(true); }; const handleDownloadUpdate = async () => { setUpdatePromptOpen(false); /* manual download */ await openUrl(OFFICIAL_RELEASE_URL); };
  ...
  return (
    <div className="app-container" data-card-mode={cardLayoutMode}>
      ...
      ) : <ClaudeTab status={claudeMonitorStatus} isTracked={trackedProvider === "claude"} onTrackClaude={handleTrackClaude} claudePollIntervalSecs={claudePollIntervalSecs} onClaudePollIntervalChange={handleClaudePollIntervalChange} claudeStopThresholdPct={claudeStopThresholdPct} onClaudeStopThresholdChange={handleClaudeStopThresholdChange} autoStopArmed={claudeAutoStopArmed} />}
      ...
      {updatePromptOpen && <CustomDialog title="Backup Recommended Before Upgrading" message={`A new version (${updateTag || "latest"}) is available. We strongly recommend backing up your data before installing the new upgrade to avoid losing accounts or settings.`} isConfirm cancelText="I'll backup now" confirmText="Download new version" confirmVariant="primary" messageAlign="left" onCancelClick={() => { setUpdatePromptOpen(false); void handleExportBackup(); }} onClose={(confirmed) => { if (confirmed) void handleDownloadUpdate(); else setUpdatePromptOpen(false); }} />}
